...
# Priors are consumed from the learned_priors snapshot, not from a TOML file.
# _write_weights used to flatten these multipliers into additive
# [detector_bonuses] for scoring.novelty_score. That path is gone in both
# directions: novelty_score no longer decides anything (emit() scores centrally
# via detect.interest), and flattening a clamped, decayed, cold-start-neutral
# multiplier into one additive nudge discarded most of what made it
# trustworthy. learn.apply.latest_stats reads the snapshot and it is applied at
# *ranking* time — the scanner's top-K and detect.interest.rank_board — where a
# prior can reorder the board but can never relax a gate.
# private/interest_weights.toml survives as hand-set config; only
# thresholds.min_novelty is still read.
...
def report(result: LearnResult) -> str:
    ...
    if result.persisted:
        lines += ["", f"Snapshot {result.run_id} written to learned_priors"]
